...
- Every subagent brief carries the browser rule: for WebGL or animated work, verify active rendering and the intended GPU/device conditions. Use headed Chrome via `launchPlacedChrome()` (`scripts/lib/launch-chrome.mjs`) when available; a supported equivalent is valid if it preserves capture and measurement conditions. Do not claim target GPU behavior from an unmatched headless, minimized, or software-rendered run. Static media may render without that path. Give browser control one owner or isolated profiles; do not assume a named tool exists.
